...
describe('the /admin/roles permission gate', () => {
  ...
  it('gates both nav rows that link here, so neither is "shown and then 404s"', () => {
    const chrome = readFileSync(join(process.cwd(), 'src', 'client', 'console-chrome.tsx'), 'utf8');

    // The admin area's own row, declared beside the permission its destination requires.
    expect(chrome).toContain("href: '/admin/roles'");
    expect(chrome).toContain('permission: PERMISSION.rbacManage');
    // The settings area's "Roles" row — live now, pointed here, and gated on the same permission.
    expect(chrome).toContain('hasPermission(permissions, PERMISSION.rbacManage)');
    // The disabled placeholder it replaced is gone outright, not kept as a fallback branch. The
    // constant is asserted absent as a DECLARATION and as a USE — the doc comment above the
    // function still names it, deliberately, to record what was deleted and why.
