...
describe('the /admin/usage permission gate', () => {
  ...
  it('reads its panel list server-side and fails LOUD on a missing entry', () => {
    ...
    // fail-loud by contract: a missing entry throws with the route named rather than rendering an
    // empty dashboard. ADR 0017 made it the shared helper for all six dashboard routes (it also
    // resolves the entry's i18n keys against the request's locale), replacing the hand-copied
    // `findPage` + `throw` this file used to pin per route — the throw is asserted once, on the
    // helper itself, below.
    expect(source).toContain('await dashboardPage(ADMIN_USAGE_ROUTE)');
