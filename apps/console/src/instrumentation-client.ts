 ...
 * `axe-core` behind it are all dropped. Verified by grepping `.next/` after `pnpm --filter console
 * build:web`; the command and its output are in `docs/knowledge/accessibility.md`. Do not turn
 * this into a runtime flag: there is no case for shipping an auditor to users.
 */
if (process.env.NODE_ENV === 'development') {
  void import('@lightbridge/ui-web/src/dev/axe-reporter').then(({ startDevA11yReporter }) =>
    startDevA11yReporter({ appName: 'console' })
  );
}
