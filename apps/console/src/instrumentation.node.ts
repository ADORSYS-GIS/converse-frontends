...
/**
 * The console's OpenTelemetry start-up. Node runtime only — reached solely from
 * `instrumentation.ts`'s `NEXT_RUNTIME === 'nodejs'` branch, via a dynamic import.
 *
 * `converse-console` is the COMPILED-IN default `service.name`: the name a trace from this image
 ...
 * saying so.
 */
startTelemetry({ serviceName: 'converse-console' });
