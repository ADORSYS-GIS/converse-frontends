 ...
 *
 * **Estate-read fast path (converse-frontends#368, "not just the one the user is bound to"):** an
 * `account`-scoped query from a session holding **`usage:read-all`** bypasses the ownership
 ...
 * permission check is the correct (and only available) substitute for a per-tenant predicate that
 ...
 *
 * `canReadAllUsage` MUST be computed server-side from the decrypted session cookie's own
 * `user.permissions` (`server/access.ts`'s `can(session, 'usage:read-all')`, the identical check
 * `app/(console)/admin/overview/page.tsx` already gates the route itself with —
 * converse-frontends#452 replaced the `isAdmin(session.user.roles)` role check this used to name,
 * and it is the same permission the backend enforces, so the two now agree by construction rather
 * than by a role that happened to carry it) — never from a
 * client-supplied header or body field. `route.ts` is the only caller and passes exactly that.
 * Omitted (`undefined`) is treated as `false` — the ordinary path is completely unchanged: it
 * still resolves and checks real ownership, so a caller who somehow reached this guard without a
 * verified permission set gets the pre-existing, unmodified behavior.
 *
 * **`scope: 'all'` (lightbridge-authz#605, the `/admin/overview` estate data path):** accepted
 * ONLY alongside `canReadAllUsage === true`, via its own fast path below — mirroring the
 * `scope: 'account'` fast path, since "all" has no per-account ownership predicate to check even
 * in principle. A `scope: 'all'` request WITHOUT the permission is refused by the existing generic
 * `isScopeOwned` fallthrough (no arm exists for `'all'`, so it fails closed exactly like an
 * unrecognised scope kind). Unlike every other scope this guard reasons about, the BACKEND now
 * independently enforces this one too — `Permission::UsageReadAll` (`usage:read-all`), read
 * directly off the bearer token, the very permission this guard is handed — so this client-side
 * check is genuinely defense-in-depth here, not the sole authorization story the way it is for
 * `scope: 'account'` (whose backend authority is only the mTLS-authenticated proxy, per the
 * estate-read fast-path note above).
 *
...
/**
 * The scopes a `usage:read-all` holder may name ANY `scope_id` under, mirroring the backend's
 * own rule verbatim (`handlers::query::query_usage`, lightbridge-authz PR #652).
 *
 ...
 * every caller — a `usage:read-all` holder included. Stated as a closed set rather than an `||`
 * chain so "which scopes does this permission unlock" has exactly one answer a test can read.
 */
...
export async function guardUsageScope(
  ...
  // ── Estate-read fast path (converse-frontends#368, widened for `user`/`project` by #448, and
  // converted from a role check to `usage:read-all` by #452) — see this function's own doc comment
  // above for why a permission bypass, not a wider per-tenant predicate, is
  ...
  // `canReadAllUsage` is trusted here BECAUSE the caller (`route.ts`) is contractually required to derive
  ...
  if (canReadAllUsage === true && ADMIN_READABLE_SCOPES.has(parsed.scope)) {
    return { ok: true };
  }

  // ── `usage:read-all` scope=all fast path (lightbridge-authz#605, the /admin/overview estate data path) —
  ...
  // `scope: 'account'` fast path immediately above, applied to the one scope where even a
  // per-tenant predicate could never exist. The backend now independently enforces this too
  // (`Permission::UsageReadAll`/`usage:read-all`, the same permission) — this guard is
  ...
  // A `scope: 'all'` request without the permission falls through unchanged to `isScopeOwned` below, which has
  // no arm for `'all'` and fails closed exactly like `'user'`/`'api_key'` already do.
  if (canReadAllUsage === true && parsed.scope === 'all') {
    return { ok: true };
  }
