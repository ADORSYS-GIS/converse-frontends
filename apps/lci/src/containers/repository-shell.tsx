...
import { DenyRepoButton } from './deny-repo-button';
 ...
 * an approve form and a deny button. It lands where `apps/console` puts `DashboardExportButton` —
 * a page-scoped group at the row's trailing edge, `align: 'end'` — because that is exactly what it
 * is: an action on the SUBJECT of the page, not on any one card in it.
 *
            ...
            children: (
              <>
                ...
                {canDeny && repo.status !== 'disabled' ? (
                  <DenyRepoButton id={id} slug={repoSlug(repo)} />
                ) : null}
              </>
            ),
          },
