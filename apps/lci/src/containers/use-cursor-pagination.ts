...
/**
 * URL-state pager for a keyset-paginated list (`GET /repositories`) — a numbered "N / M" display
 * over a cursor whose actual boundary is a row, not a position.
 *
 ...
 * returned; `page` itself is never sent anywhere. `shallow: false` on every param: paging drives a
 * real server-side fetch (`listRepositoriesPage`), so a URL-only change (nuqs' default
 * `shallow: true`) would update the address bar without ever re-invoking the Server Component
 * that owns the data.
 *
...
export function useCursorPagination({
  ...
  // `Pagination` only ever calls this with `current - 1` or `current + 1`, so the sign of the
  // move says which cursor to send. `next`/`prev` are null exactly when the corresponding
  // direction is unavailable — both come from the same response as `total` — so a move landing
  // here always has the cursor it needs; the `&& next`/`&& prev` guards are a graceful no-op for
  // the rare case a concurrent change makes that momentarily untrue.
  ...
  return { current, pageCount, goToPage, reset };
}
