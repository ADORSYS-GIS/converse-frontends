 ...
 * fail.
 *
 * **The Edge runtime is deliberately NOT instrumented.** `src/proxy.ts` is this app's only
 * Edge-resident code; it reads the session cookie and either passes the request through or
 * redirects it to the sign-in route, calling nothing and awaiting nothing. Instrumenting it would
 * need `@vercel/otel` (the only SDK with an Edge path) for the sake of a span with no children, on
 * a platform this estate does not deploy to. The trace still begins at the Node server span the
