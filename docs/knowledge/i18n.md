...
| Switcher          | `apps/console/src/i18n/locale-switcher.tsx` — a `SelectField` dropdown, mounted in the sidebar footer ("Language") and on `/settings/info` |
...
export builder both stay ignorant of i18n. Storybook's `Dashboard/FromSpec` reads the same YAML and
