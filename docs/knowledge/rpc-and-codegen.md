...
| `packages/authz-rpc/src/codec.ts`                           | Committed                        | The single wire codec — `@cratestack/cbor`-based CBOR (`getCborCodec()`, resolved once via `ensureCborCodecReady()`), used identically by `apps/console` and `apps/self-service`; `JsonCodec` also lives here as a unit-test-only alternative |
...
`allowedModels: ['gpt-4']` must round-trip as the bare array through both the `@cratestack/cbor`-
based codec and `JsonCodec`, and a dedicated test (_"fails if allowedModels is ever externally tagged again"_)
