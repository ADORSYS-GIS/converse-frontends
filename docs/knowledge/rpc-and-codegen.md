...
flowchart TD
    subgraph committed["Committed to git"]
        schema["authz.cstack schema<br/>about 684 lines, synced from lightbridge-authz"]
...
| Path                                                        | Status                           | Role                                                                                                                                                           |
| ----------------------------------------------------------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/authz-rpc/schema/authz.cstack`                    | Committed                        | Source of truth — a cratestack schema, ~684 lines, copied from the backend repo                                                                                |
| `packages/authz-rpc/generated/`                             | **Gitignored** (`.gitignore:34`) | 100% codegen output. Never hand-edit; regenerated by `pnpm install`.                                                                                           |
| `packages/authz-rpc/src/codec.ts`                           | Committed                        | JSON/CBOR wire codecs                                                                                                                                          |
...
sequenceDiagram
    ...
    Client->>Backend: fetch POST, body encoded by codec, CBOR in prod, JSON in dev
