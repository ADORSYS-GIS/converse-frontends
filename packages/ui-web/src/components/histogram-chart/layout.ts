 ...
 * `spend-series-chart/domain.ts`'s `collectYDomain`: bars grow from a zero
...
/**
 * Map bins to pixel bars against `xScale`, leaving a `gap`px seam between
 * adjacent bars (the dataviz skill's "surface gap, not a border" bar mark
 * rule -- touching bars need daylight between them).
 *
 ...
 * -- same subsampling shape as `spend-series-chart/component.tsx`'s x-tick
