 ...
 *
 * Closed-form derivation: choosing `spacing = plotHeight / (rowCount +
 * overlap)` and reserving `topPad = spacing * overlap` above the first
 * baseline makes the first row's peak land exactly at `y = 0` (never clipped
 * above the plot area) and the last row's baseline land exactly at `y =
 * plotHeight` (flush with the bottom axis), for any `rowCount >= 1`.
 *
