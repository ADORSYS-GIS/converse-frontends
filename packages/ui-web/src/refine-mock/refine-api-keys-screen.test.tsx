...
describe('RefineApiKeysScreen', () => {
  it('adapts useTable loading/data state into ApiKeysPage props: skeleton while loading, then the live ledger', async () => {
    ...
    await waitFor(() => expect(screen.getByText('ci-deploy')).toBeInTheDocument(), { timeout: 3000 });
  });
  ...
  it('revoking a key through the confirm dialog flows through useUpdate and flips its STATUS cell', async () => {
    ...
    fireEvent.change(within(dialog).getByLabelText('Type "ci-deploy" to confirm'), { target: { value: 'ci-deploy' } });
  ...
  it('adapts a getList failure into ApiKeysPage error props (ErrorLine + Retry)', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('alert')).toHaveTextContent('Failed to load keys for this project.'), {
      timeout: 3000,
    });
  });
});
